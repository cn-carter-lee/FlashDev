package elements.axis
{
	...
	public class YAxisBase extends Sprite
	{
		...
		// called once the sprite has been added to the stage so now it has access to the stage		
		protected function _init(json:Object, name:String, style:Object):void
		{
			this.style = style;
			
			if (json[name])
				object_helper.merge_to_default(json[name], this.style);
			
			this.colour = Utils.get_colour(style.colour);
			this.grid_colour = Utils.get_colour(style['grid-colour']);
			this.grid_colour = 0xFF0000;
		...
		{
			// Set opacity for the first line to 0 (otherwise it overlaps the x-axel line) Bug? Does this work on graphs with minus values?
			var i2:Number = 0;
			var i:Number;
			var y:Number;
			var lbl:Object;
			...
			this.graphics.drawRect(int(pos), // <-- pixel align
				sc.top, this.stroke, sc.height);
		...
		private function draw_grid_lines(steps:Number, min:Number, max:Number, right:Boolean, sc:ScreenCoords):void
		{
			...
			else
			{
				...
				for (var i:Number = min; i <= max; i += steps)
				{
					...
					this.graphics.drawRect(int(sc.left), int(y), // <-- make sure they are pixel aligned (2.5 - 3.5 == fuzzy lines)
						sc.width, 1);
					this.graphics.endFill();
				}
			}
