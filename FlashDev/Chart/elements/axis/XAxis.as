package elements.axis
{
	...
	public class XAxis extends Sprite
	{
		...
		{
			// default values
			this.style = {stroke: 2, 'tick-height': 0, colour: '#000000', offset: true, 'grid-colour': '#cccccc', 'grid-visible': false, '3d': 0, steps: 1, min: 0, max: null};
			
			if (json != null)
				object_helper.merge_to_default(json.x_axis, this.style);
			...
			// the X Axis labels *may* require info from this.obs			
			if (!this.range_set())
			{
				// the user has not told us how long the X axis is, so we figure it out:
				...
				{
					// X Axis labels used, even so, make the chart big enough to show all values					
		...
		// a little hacky, but we inspect the labels to see if we need to display user custom ticks
		...
		private function calcSteps():void
		{
			...
			else
			{
				...
				if ((Math.abs(xRange) / this.steps) > 250)
					this.steps = xRange / 250;
		...
		public function get_height():Number
		{
			if (this.three_d)
			{
				// 12 is the size of the slanty 3D part of the X axis				
