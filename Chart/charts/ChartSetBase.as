package charts
{
	...
	public class ChartSetBase extends Sprite
	{
		...
		// hold the Element values, for lines this is an array of string Y values, 
		// for Candle it is an array of string 'high,open,low,close' values, for scatter it is 'x,y' etc...
		...
		// whatever sets of data that *may* be attached to the right Y Axis call this to see if they are attached to it or not. All lines, area and bar charts call this.
		protected function which_axis_am_i_attached_to(data:Array, i:Number):Number
		{
			// some data sets are attached to the right Y axis (and min max), in the future we may support many axis
		...
		// Line and bar charts will normally only have one Element at any X position, but when using Radar axis you may get many at any give X location. Scatter charts can have many items at the same X position
		public function get_closest(x:Number, y:Number):Array
		{
		...
		{
			var closest_distance:Number = Number.MAX_VALUE;
			var p:flash.geom.Point;
			var x_postion:Number;
			var tmp_distance_x:Number;
			
			for (var i:Number = 0; i < this.numChildren; i++)
			{
				// some of the children will will mask Sprites, so filter those out:
				if (this.getChildAt(i) is Element)
				{
					var element:Element = this.getChildAt(i) as Element;
					p = element.get_mid_point();
					tmp_distance_x = Math.abs(x - p.x);
					if (tmp_distance_x < closest_distance)
					{
						closest_distance = tmp_distance_x;
						x_postion = p.x;
					}
				}
			}
		...
		protected function get_all_at_this_x_pos(x:Number):Array
		{
			...
			for (var i:Number = 0; i < this.numChildren; i++)
			{
				...
				if (this.getChildAt(i) is Element)
				{
					...
					// Point elements are invisible by default.Prevent invisible points from showing tooltips For scatter line area
					//if (e.visible) {
					p = e.get_mid_point();
					if (p.x == x)
						tmp.push(e);
		...
		// scatter charts may have many Elements in the same x, y location
		...
		// this is a backup function so if the mouse leaves the movie for some reason without raising the mouse out event (this happens if the user is wizzing the mouse about)
		...
		// index of item (bar, point, pie slice, horizontal bar) may be used to look up its X value (bar,point) or Y value (H Bar) or used as the sequence number (Pie)
		...
		public function add_values():void
		{
			...
			{
				var tmp:Element;
				// TODO: fix or document what is happening in link-null-bug.txt,filter out the 'null' values
				if (val != null)
				{
		...
		// See ObjectCollection tooltip_replace_labels,@param	labels
