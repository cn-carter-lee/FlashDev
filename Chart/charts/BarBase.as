package charts
{
	...
	public class BarBase extends ChartSetBase
	{
		...
		public function BarBase(json:Object, group:Number)
		{
			...
			var root:Properties = new Properties({values: [], colour: '#3030d0', text: '', // <-- default not display a key
					'font-size': 12, tip: '#val#<br>#x_label#', alpha: 0.6, 'on-click': false, 'axis': 'left'});
		...
		protected function get_on_show(json:Object):Properties
		{
			...
			var on_show_root:Properties = new Properties({type: "none", //"pop-up",
					cascade: 3, delay: 0});
		...
		protected function get_element_helper_prop(value:Object):Properties
		{
			...
			var default_style:Properties = new Properties({colour: this.props.get('colour'), tip: this.props.get('tip'), alpha: this.props.get('alpha'), 'on-click': this.props.get('on-click'), axis: this.props.get('axis'), 'on-show': this.on_show});
