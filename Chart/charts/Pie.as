package charts
{
	...
	public class Pie extends ChartSetBase
	{
		...
		public function Pie(json:Object)
		{
			...
			this.style = {colours: ["#900000", "#009000"] // slices colours
				}
...
//			this.Elements= new Array();
			...
			for each (val in this.values)
			{
				...
				if (slice_angle >= 0)
				{
					...
					this.addChild(this.add_slice(i, slice_start, slice_angle, val, // <-- NOTE: val (object) NOT value (a number)
						t, this.colours[(i % this.colours.length)]));
		...
		{			
			// Properties chain:   pie-slice -> calculated-stuff -> pie
			// calculated-stuff:
			var calculated_stuff:Properties = new Properties({colour: colour, // <-- from the colour cycle array
		...
		public override function resize(sc:ScreenCoordsBase):void
		{
			...
			for (i = 0; i < this.numChildren; i++)
			{
				...
				if (ticAngle >= 270)
				{
					...
					if ((rightBottomTicAngle < 0) || ((rightBottomTicAngle > 90) && (rightBottomTicAngle < ticAngle)))
					{
				...
				else if (ticAngle <= 90)
				{
					...
					if ((rightTopTicAngle > 360) || ((rightTopTicAngle <= 90) && (ticAngle < rightBottomTicAngle)))
					{
						rightTopTicAngle = ticAngle;
						rightTopTicIdx = i;
					}
