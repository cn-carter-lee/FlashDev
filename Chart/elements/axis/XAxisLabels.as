package elements.axis
{
	...
	public class XAxisLabels extends Sprite
	{
		...
		function XAxisLabels(json:Object)
		{
			...
			this.style = {rotate: 0, visible: null, labels: null, text: '#val#', // <-- default to display the position number or x value
					steps: null, // <-- null for auto labels
					size: 10, align: 'auto', colour: '#000000', "visible-steps": null};
			
			// cache the text for tooltips
			this.axis_labels = new Array();
			
			if ((json.x_axis != null) && (json.x_axis.labels != null))
				object_helper.merge_to_default(json.x_axis.labels, this.style);
			...
			{				
				...
				{
					var tmpStyle:Object = {};
					object_helper.merge_to_default(this.style, tmpStyle);
		...
		// we were not passed labels and need to make them from the X Axis range
		public function auto_label(range:Range, steps:Number):void
		{
			...
			if (this.need_labels)
			{
				...
				if (this.style.steps != null)
					lblSteps = this.style.steps;
				
				// force max of 250 labels 
				if (Math.abs(range.count() / lblSteps) > 250)
					lblSteps = range.count() / 250;
				...
				object_helper.merge_to_default(this.style, tempStyle);
				var lblCount:Number = 0;
		...
		public function add(label:Object, style:Object):void
		{
			...
			var label_style:Object = {colour: style.colour, text: style.text, rotate: style.rotate, size: style.size, align: style.align, visible: style.visible, x: style.x};
